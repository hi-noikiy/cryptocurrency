...
public class BotTasks {
    ...
    public void checkTask() {
        ...
        logger.info("usdt : {}", cashTotal);
        logger.info("btc : {}", exchangeRemote.getTradeAmount("btc"));
        logger.info("eos : {}", exchangeRemote.getTradeAmount("eos"));
        logger.info("neo : {}", exchangeRemote.getTradeAmount("neo"));


        if (cashTotal > 10) {
            int cashPiece = 3 - ExchangeRemote.TRADE_STATUS.buyTotal();
            String cash = String.valueOf((cashTotal - 1) / cashPiece);
